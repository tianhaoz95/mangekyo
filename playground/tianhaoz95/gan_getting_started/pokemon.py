...
def build_generator_model():
    ...
    # From a vector of 64*64*64 to (64,64,64)
    model.add(keras.layers.Dense(64*64*64, use_bias=False, input_shape=(256,)))
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.LeakyReLU())
    model.add(keras.layers.Reshape((64, 64, 64)))
    # From (64, 64, 64) to (64, 64, 32)
    model.add(keras.layers.Conv2DTranspose(
        32, (5, 5), strides=1, padding='same', use_bias=False))
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.LeakyReLU())
    # From (64, 64, 32) to (128, 128, 16)
    model.add(keras.layers.Conv2DTranspose(
        16, (5, 5), strides=2, padding='same', use_bias=False))
    model.add(keras.layers.BatchNormalization())
    model.add(keras.layers.LeakyReLU())
    # From (128, 128, 16) to (256, 256, 3)
    model.add(keras.layers.Conv2DTranspose(
